...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
    ...
    SET 
      ...
      role = CASE
        WHEN (SELECT raw_user_meta_data->>'role' FROM auth.users WHERE id = u.id) = 'coordinator' THEN 'coordinator'
        WHEN (SELECT raw_user_meta_data->>'role' FROM auth.users WHERE id = u.id) = 'teacher' THEN 'teacher'
        WHEN (SELECT raw_user_meta_data->>'role' FROM auth.users WHERE id = u.id) = 'parent' THEN 'parent'
        WHEN (SELECT raw_user_meta_data->>'role' FROM auth.users WHERE id = u.id) = 'student' THEN 'student'
        ELSE u.role
      END
    WHERE u.email IN ('coordenador@teste.com', 'professor@teste.com', 'pais@teste.com', 'aluno@teste.com');
    
    RAISE NOTICE 'Registros em public.users atualizados';
  ELSE
    RAISE NOTICE 'Tabela public.users não existe, pulando atualização';
  END IF;
END $$;
