...
export default function GerenciarAlunosPage() {
  ...
  return (
    <div className="relative flex h-auto min-h-screen w-full flex-col bg-background-dark text-text-dark overflow-x-hidden">
      {/* Header */}
      <div className="sticky top-0 z-10 w-full bg-background-dark safe-area-top">
        <div className="flex items-center p-3 sm:p-4">
          ...
          <h1 className="text-slate-900 dark:text-white text-lg sm:text-2xl font-bold leading-tight flex-1 text-center px-2 truncate">
            Gerenciar Alunos
          </h1>
        </div>
        {/* Search Bar */}
        <div className="px-3 sm:px-4 pb-3 sm:pb-4">
          <label className="flex flex-col min-w-40 h-12 sm:h-14 w-full">
            <div className="flex w-full flex-1 items-stretch rounded-lg h-full">
              <div className="text-slate-400 dark:text-[#8E8E93] flex border-none bg-slate-200/10 dark:bg-[#2C2C2E] items-center justify-center pl-3 sm:pl-4 rounded-l-lg border-r-0">
      ...
      {/* Main Content */}
      <main className="flex-grow px-3 sm:px-4 pb-24 sm:pb-28 overflow-y-auto smooth-scroll">
        {/* Sort Options */}
        ...
        <div className="flex flex-col gap-2 sm:gap-3">
          {loading ? (
            ...
            filteredAndSortedAlunos.map((aluno) => (
              <div
                key={aluno.id}
                className="flex cursor-pointer items-center gap-3 sm:gap-4 bg-slate-100/10 dark:bg-[#2C2C2E] p-3 sm:p-4 min-h-[64px] sm:min-h-[72px] rounded hover:bg-slate-100/20 active:bg-slate-100/30 transition-colors touch-manipulation"
                onClick={() => router.push(`/coordenador/alunos/${aluno.id}`)}
              >
                <div
                  className="bg-center bg-no-repeat aspect-square bg-cover rounded-full h-10 w-10 sm:h-12 sm:w-12 shrink-0"
                  style={{ backgroundImage: `url('${aluno.avatar}')` }}
                />
                <div className="flex flex-col justify-center overflow-hidden flex-1 min-w-0">
                  <p className="text-slate-900 dark:text-white text-sm sm:text-base font-medium leading-normal truncate">
                    {aluno.nome}
                  </p>
                  <p className="text-slate-500 dark:text-[#8E8E93] text-xs sm:text-sm font-normal leading-normal truncate">
      ...
      <div className="fixed bottom-0 right-0 w-full p-3 sm:p-6 pointer-events-none safe-area-bottom">
        <div className="flex justify-end">
          <Button
            onClick={() => router.push('/coordenador/alunos/novo')}
            className="flex max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-12 sm:h-14 shadow-lg bg-primary text-white text-sm sm:text-base font-bold leading-normal tracking-wide min-w-0 gap-2 sm:gap-3 px-4 sm:px-5 pointer-events-auto touch-manipulation"
          >
