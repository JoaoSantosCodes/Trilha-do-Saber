...
export default function BuscarAmigosPage() {
  ...
  return (
    <div className="relative flex h-screen w-full flex-col bg-background-dark">
      {/* Header */}
      <div className="sticky top-0 z-10 flex items-center bg-background-dark p-4 pb-2 justify-between">
      ...
      <div className="px-4 py-3">
        <label className="flex flex-col min-w-40 h-12 w-full">
          <div className="flex w-full flex-1 items-stretch rounded-xl h-full">
            <div className="text-gray-400 flex border-none bg-zinc-800 items-center justify-center pl-4 rounded-l-xl border-r-0">
            ...
            <input
              ...
              className="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-xl text-white focus:outline-0 focus:ring-0 border-none bg-zinc-800 focus:border-none h-full placeholder:text-gray-400 px-4 rounded-l-none border-l-0 pl-2 text-base font-normal leading-normal"
            />
          </div>
        </label>
      </div>
      ...
      <div className="flex px-4 py-3">
        <div className="flex h-10 flex-1 items-center justify-center rounded-xl bg-zinc-800 p-1">
          <button
          ...
          {loading ? (
            <LoadingSkeleton type="list" count={5} />
              ...
              users.map((user) => (
                <div
                  key={user.id}
                  className="flex items-center gap-4 rounded-xl bg-zinc-800/50 p-4"
                >
                  <Image
                  ...
                  {user.status === 'available' ? (
                    <Button
                      ...
                      className="min-w-[84px] h-10 px-4 bg-primary text-black text-sm font-bold gap-2 disabled:opacity-50"
                    >
                      {isAdding === user.id ? (
                    ...
                    <button
                      disabled
                      className="flex min-w-[84px] cursor-not-allowed items-center justify-center gap-2 overflow-hidden rounded-full h-10 px-4 bg-green-600 text-white text-sm font-bold leading-normal tracking-[0.015em]"
                    >
                    ...
                    <button
                      disabled
                      className="flex min-w-[84px] cursor-not-allowed items-center justify-center gap-2 overflow-hidden rounded-full h-10 px-4 bg-zinc-700 text-gray-400 text-sm font-bold leading-normal tracking-[0.015em]"
                    >
          ...
          <div className="flex flex-col gap-4 p-4">
            {friendRequests.length > 0 ? (
              friendRequests.map((request) => (
                <div
                  key={request.id}
                  className="flex flex-col gap-3 rounded-xl bg-[#2D2D2D]/50 p-4"
                >
